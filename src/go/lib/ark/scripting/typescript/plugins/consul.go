...
func NewConsulPlugin(opts stdlib.Options) NativePlugin {
	return NativePlugin{
		Name: "@mantl/sre/consul",
		Module: typescript.Module{
			"default": generateConsulManifest(opts),
		},
	}
}

func generateConsulManifest(opts stdlib.Options) func(call goja.FunctionCall) goja.Value {
	return helpers.NewGojaErrHandler(opts.Runtime, func(call goja.FunctionCall) (goja.Value, error) {
		pluginOpts := pkg.ConsulOptions{}
		...
		manifest, err := pkg.NewConsulManifest(pluginOpts)
		if err != nil {
