...
func (w *Walker) Update(g *AcyclicGraph) {
	ctxLog.Tracef("dag/walk: updating graph")
	var v, e *Set
	if g != nil {
	...
	for _, raw := range newVerts.List() {
		...
		ctxLog.Tracef("dag/walk: added new vertex: %q", VertexName(v))
		w.vertices.Add(raw)
	...
	for _, raw := range oldVerts.List() {
		...
		ctxLog.Tracef("dag/walk: removed vertex: %q", VertexName(v))
	...
	for _, raw := range newEdges.List() {
		...
		waiterInfo, ok := w.vertexMap[hashcode(waiter)]
		if !ok {
		...
		depInfo, ok := w.vertexMap[hashcode(dep)]
		if !ok {
			// Vertex doesn't exist... shouldn't be possible but ignore.
			continue
		}
		...
		waiterInfo.deps[hashcode(dep)] = depInfo.DoneCh

		// Record that the deps changed for this waiter
		changedDeps.Add(waiter)

		ctxLog.Tracef(
			"walk: added edge: %q waiting on %q",
	...
	for _, raw := range oldEdges.List() {
		...
		ctxLog.Tracef(
			"walk: removed edge: %q waiting on %q",
			VertexName(waiter), VertexName(dep))
		w.edges.Delete(raw)
	}
	...
	for _, raw := range changedDeps.List() {
		v := raw.(Vertex)
		info, ok := w.vertexMap[hashcode(v)]
		if !ok {
			// Vertex doesn't exist... shouldn't be possible but ignore.
			continue
		}
		...
		ctxLog.Tracef(
			"walk: dependencies changed for %q, sending new deps",
			VertexName(v))
	...
	for _, raw := range newVerts.List() {
		v := raw.(Vertex)
		go w.walkVertex(v, w.vertexMap[hashcode(v)])
	}
}
...
func (w *Walker) walkVertex(v Vertex, info *walkerVertex) {
	...
	if depsSuccess {
		ctxLog.Tracef("dag/walk: visiting %q", VertexName(v))
		diags = w.Callback(v)
	} else {
		ctxLog.Tracef("dag/walk: upstream of %q errored, so skipping", VertexName(v))
	...
	DepSatisfied:
		for {
			...
			case <-time.After(time.Second * 5):
				ctxLog.Tracef("dag/walk: vertex %q is waiting for %q",
