...
result format may change in future versions of Ark, based on feedback.
...
representation of a value use [`jsonencode`](./jsonencode.md) instead; its
...
Because the YAML format cannot fully represent all of HCL's
types, passing the `yamlencode` result to `yamldecode` will not produce an
identical value, but HCL automatic type conversion rules
...
[`jsonencode`](./jsonencode.md) instead: YAML flow-style is a superset
of JSON syntax.

## Related Functions

- [`jsonencode`](./jsonencode.md) is a similar operation using JSON instead
  of YAML.
- [`yamldecode`](./yamldecode.md) performs the opposite operation, _decoding_
