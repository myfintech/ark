...
[`regexall`](./regexall.md) and test that the result has length greater than
zero.
...
regular expression pattern as a literal quoted string in HCL, 
the quoted string itself already uses backslash `\` as an escape
...
- [`regexall`](./regexall.md) searches for potentially multiple matches of a given pattern in a string.
- [`replace`](./replace.md) replaces a substring of a string with another string, optionally matching using the same regular expression syntax as `regex`.
